# Brand Identity System: Qteria

> **Product**: Qteria - AI-Powered Document Validation for TIC Industry
...
This visual identity system expresses **professional competence with approachable warmth** to **TIC notified bodies and certification professionals** through design. Every visual element communicates **transparency, quality, and domain expertise** while supporting the journey value: **evidence-based AI validation that saves time and prevents embarrassment**.

### Brand Personality (from Brand Strategy)

**Character**: The competent quality management professional in her early 30s - office nerd who loves details and precision. Balkan beauty. The person you want reviewing your documents because she catches everything and explains it clearly.

- **Personality Traits**: Professional, Direct, Trustworthy, Organized, Supportive
- **Core Values**: Quality Over Speed (but we deliver both), Transparency, Simplicity, Domain Expertise, Partnership
- **Visual Direction**: Clean, organized, trustworthy - modern but not trendy, professional but not cold, subtle warmth prevents generic enterprise feel
- **Target Audience**: Project Handlers (daily users running 15-20 assessments) and Process Managers (defining validation workflows) at TIC notified bodies

### Design Philosophy

**Clarity Through Structure**

Our visual identity balances professional credibility with approachable expertise. Like a well-organized office where everything has its place, the identity communicates competence through clean structure while subtle warmth (Balkan heritage) prevents it from feeling like generic enterprise software. Evidence-based validation requires evidence-based design - every visual choice serves clear communication, not decoration.

**Core Design Principle**: If it doesn't help users validate documents faster or trust results more, it doesn't belong in the identity.

---

## Logo Concepts

### Concept A: The Evidence Mark (Lettermark + Symbol)

**Type**: Combination Mark (Lettermark "Q" + Wordmark "teria")
...
### Concept B: The Document Thread (Wordmark + Abstract Symbol)

**Type**: Combination Mark (Abstract Symbol + Wordmark)
...
### Concept C: The Quality Q (Pure Wordmark with Distinction)

**Type**: Wordmark (Text Only with Visual Distinction)
...
- ✅ Color split creates interest without complexity
...
### Concept D: The Certificate Badge (Pictorial Mark + Word)

**Type**: Combination Mark (Pictorial Icon + Wordmark)
...
### Concept E: The Insight Line (Minimalist Abstract)
...
- Total form is contained, simple, one continuous line
...
- Gradient trends can date (flat color fallback needed for longevity)

---

## Recommended Logo: Concept A - The Evidence Mark

**Why this concept wins**:

✅ **Best expresses brand personality**: The Q as quality badge + magnifying lens perfectly embodies "competent professional who catches everything." Geometric precision = organized expertise. Checkmark tail integration = direct communication (we validate). Terracotta accent = Balkan warmth differentiating from cold enterprise tools.

✅ **Most appealing to journey audience**: TIC professionals immediately recognize quality/validation symbolism (badge + checkmark). The "Q" is distinctive and memorable - builds brand equity through unique letterform. Process Managers and Project Handlers both connect with clarity (no abstract interpretation needed).

✅ **Versatile across all touchpoints**: The Q letterform works standalone (favicons, app icons at 24px+) or with full wordmark (headers, business cards). Scales from 16px single-color favicon to billboard. Horizontal layout for headers, stacked for square formats.

✅ **Distinctive from competitors**: Not a generic document icon or floating checkmark. The Q letterform is ownable IP. Terracotta accent breaks the "all blue" compliance tool pattern while maintaining professionalism. "Q for Quality" is intuitive but executed uniquely.
...
- Large formats: All detail visible, badge form impactful

**Journey-Brand Alignment**:

- **Communicates "evidence-based validation" visually**: The Q as magnifying lens = looking closely at documents for evidence. The checkmark tail = validation result. The circular badge = quality gate (documents must pass through).

- **Appeals to Project Handlers' emotional state**: When they face quality anxiety (errors slipping through to Certification Person), the quality badge logo communicates authority and thoroughness. "This tool catches what I might miss."

- **Reinforces brand promise**: "Clear, accurate, respectful of expertise" = The logo is clear (simple Q form), accurate (quality badge = precise validation), respectful (professional execution, not playful or condescending).

**Implementation recommendation**: Start with **Concept A** and create logo files immediately. This identity can launch with MVP. If user testing reveals strong feedback for different direction, revisit Concept C (wordmark simplicity) or Concept B (document thread storytelling) in future.
...
- **Use**: Primary application across all brand materials (website, app, marketing, product)
...
- **Digital**:
  ...
  - Favicon (browser tab): 16px - use simplified single-color Q
- **Print**:
  - Full logo: 0.6 inches height minimum
  - Icon-only: 0.4 inches minimum
- **Below minimum**: Use icon-only Q in single color (blue or gray)
...
- **Use**:
  ...
  - Social media profile picture (when "Qteria" appears in username/handle)
  - Loading spinners (Q can rotate as progress indicator)
- **Works at**: 24px+ for two-color version, 16px+ for single-color
...
**Clearspace Rule**: Maintain clearspace equal to **0.5x the height of the Q** on all sides of the logo.
...
**Why**: Clearspace ensures the logo has breathing room and isn't crowded by other UI elements, text, or competing visuals. This maintains visibility, impact, and professional presentation.

**Exception**: In navigation bars or space-constrained UI, clearspace can reduce to 0.3x Q height, but never less.
...
- Brand accents in marketing materials (section dividers, pull quotes)
- **NOT for**: Error states (use semantic red), success states (use semantic green), primary CTAs (use blue)
...
- Gray-600 on white: 7.4:1 contrast (WCAG AAA pass)
...
- **RGB**: rgb(34, 197, 94)
- **Usage**:
  - Criteria status badge ("PASS" label background green-50, text green-700)
  - Checkmark icons (✓) for passed criteria
...
- **RGB**: rgb(234, 179, 8)
- **Usage**:
  - Criteria status badge ("UNCERTAIN" label background yellow-50, text yellow-700)
  - Warning icons (⚠) for ambiguous results
...
- **RGB**: rgb(239, 68, 68)
- **Usage**:
  - Criteria status badge ("FAIL" label background red-50, text red-700)
  - X mark icons (✗) for failed criteria
  - Error messages, form validation errors
  - Destructive action confirmations ("Delete Workflow")

**Rationale**: Standard traffic light pattern (green/yellow/red) is universally understood for pass/uncertain/fail. Maintains consistency with design system. Semantic colors are always paired with icons (✓ ⚠ ✗) to ensure accessibility (never color-only communication).
...
4. **Dark Mode (Reversed)**:
   - White + Blue-400 (lighter blue for dark backgrounds) + Gray-900 background
   - Use: Dark website sections, dark mode app UI (future), video overlays
...
- **5%**: Terracotta warm accent (logo, special highlights)

---

## Typography System

### Primary Brand Font (Headlines, Logo, UI)
...
- **Weights available**:
  ...
  - 700 Bold (rare - strong emphasis, "FAIL" labels)
...
- **UI elements**: Inter Medium 500 for labels/buttons, Regular 400 for inputs/content
...
- **Variable font efficiency**: Single file for weights 400-700 reduces load time (critical for Step 3 results page with many criteria).
- **Industry standard for B2B tools**: Stripe, Linear, GitHub, Vercel use Inter - signals quality and modernity to TIC professionals.
...
### Monospace Font (Technical Details)

**Font**: **JetBrains Mono** (for document references, technical data)
...
- **Pairs well without competing**: Monospace clearly signals "technical detail" vs. Inter's general content

**Character**: Technical, precise, functional. The ligatures (optional) add subtle sophistication without sacrificing clarity.
...
- AI reasoning: **sm** (14px) - Explanation paragraph
...
- Inverted: White on blue-600 (5.2:1 contrast), white on gray-900 (18:1 contrast)
...
- **Usage**:
  - Marketing page hero sections (adds depth without distraction)
  - Empty states (break up large white spaces)
  - **NOT in app UI** (keep product interface clean, pattern-free)
- **Connection to brand**: Organized, structured (grid = quality management systems), minimal (doesn't compete with content)

**Avoid**: Complex patterns, gradients, textures that compete with content. Brand values simplicity - patterns should be felt, not seen.
...
- **Weight**: 2px stroke
- **Corners**: Rounded (matches Inter's subtle roundness)
- **Complexity**: Minimal detail - icons must be recognizable at 16px
...
- **AlertTriangle**: Triangle with ! (UNCERTAIN status, warnings)
- **FileText**: Document icon (file lists, workflow buckets)
- **Search**: Magnifying glass (search/filter inputs)
- **Settings**: Gear icon (workflow configuration)
- **User**: Avatar icon (user profile, settings)
- **ChevronDown**: Down arrow (expandable cards, dropdowns)
...
- Size: 16px (inline with text), 20px (buttons, cards), 24px (section headers)

---

### Photography Style

**Aesthetic**: **Bright, Professional, Authentic**
...
- ✅ **Diverse representation**: Age, gender, ethnicity - TIC industry is global, visuals should reflect that
- ✅ **Composition**: Clean, organized environments. Clutter-free desks, well-organized offices (matches brand value of organization/clarity).
...
- Notified body office environment (clean, professional, but lived-in)

**Color Grading**: Slight desaturation (10-15%), warm white balance (matches terracotta accent), high clarity (shows detail/precision).

**Usage**: Marketing website hero sections, case studies, about page. **NOT in app UI** (product stays minimal, photography reserved for marketing/brand storytelling).

---

### Illustration Style (Optional - Future Use)

**Style**: **Line illustrations** (matches icon style, but more detailed)

- **Aesthetic**: Simple line drawings in 2-3 colors (blue, terracotta, gray)
- **Detail level**: Moderate - more detail than icons, but not realistic/complex
- **Use cases**:
  - Empty states ("No workflows yet" - illustration of document with Q badge)
  - Onboarding screens (illustrate key concepts: workflow creation, document upload, AI validation)
...
**If implementing later**: Use Balkan illustrator (supports brand heritage), ensure illustrations match line icon aesthetic (consistent stroke weight, rounded corners, minimal detail).
...
**Rationale**: Minimal graphic elements keep focus on content (evidence-based validation results). Subtle shadows and rounded corners add warmth without undermining professionalism.
...
- **Business cards**: Top left (front), small bottom right (back)
...
3. **Add effects**:
   ...
   - No gradients overlaid on logo (unless specified in approved versions)
...
9. **Combine with other logos improperly**:
   ...
   - Fix: Use proper spacing (at least 0.5x Q height between logos)

10. **Use outdated versions**:
    - Always use current logo version (1.0 as of Nov 2025)
    - Wrong: Using old logo after rebrand/update
    - Fix: Download latest logo files from brand asset repository
...
4. Verify focus indicators visible on all backgrounds
5. Test with color blindness simulators (Stark plugin, Chrome DevTools)
...
2. **PNG** (Portable Network Graphics):
   ...
   - Resolution:
     - Web/screen: 2x retina (e.g., 200px display at 100px for crisp rendering)
...
5. **ICO** (Icon format):
   - Favicon for browsers (legacy support)
   - Sizes: 16px, 32px, 48px in single .ico file
   - Use: Browser tab icons (though modern browsers support SVG favicons)

**Naming Convention**:

```
[product-name]-logo-[variation]-[color].{format}

Examples:
qteria-logo-horizontal-color.svg
qteria-logo-horizontal-color@2x.png (retina)
qteria-logo-horizontal-blue.svg (single color)
qteria-logo-horizontal-reversed.svg (white on dark)
qteria-logo-stacked-color.svg
qteria-logo-icon-only-color.svg
qteria-logo-icon-only-blue.png (favicon)
qteria-favicon-16.png
qteria-favicon-32.png
qteria-favicon.ico (multi-size)
qteria-app-icon-1024.png (iOS/Android app store)
```
...
/brand-assets
  /logo
    /svg
      qteria-logo-horizontal-color.svg
      qteria-logo-horizontal-blue.svg
      qteria-logo-horizontal-reversed.svg
      qteria-logo-stacked-color.svg
      qteria-logo-icon-only-color.svg
    /png
      /1x (96 DPI)
        qteria-logo-horizontal-color-512px.png
      /2x (retina, 192 DPI)
        qteria-logo-horizontal-color-512px@2x.png
      /print (300 DPI)
        qteria-logo-horizontal-color-300dpi.png
    /favicon
      qteria-favicon-16.png
      qteria-favicon-32.png
      qteria-favicon.ico
      qteria-favicon.svg (modern browsers)
    /app-icon
      qteria-app-icon-1024.png (iOS App Store)
      qteria-app-icon-512.png (Android Play Store)
  /guidelines
    qteria-brand-identity-guidelines.pdf
```
...
- **Spacing**: 16px padding around logo
- **Interaction**: Logo links to homepage, no hover state needed (standard pattern)
- **Mobile behavior**: Logo remains 32px height, wordmark stays visible (Q + "teria")
- **Dark mode** (future): Use reversed logo (white Q + white wordmark) on gray-900 or blue-900 background
...
- **Background**:
  ...
  - Option B: White background, full-color Q (blue ring + terracotta tail)
  - Recommended: Option B (brand colors visible at glance in app grid)
- **Size**:
  - iOS: 1024×1024px (App Store requirement)
  - Android: 512×512px (Play Store requirement)
- **Padding**: 10% internal padding (prevent cropping by system masks)
- **Corner radius**: Let system apply (iOS rounds automatically, Android supports adaptive icons)
...
- **Sizes**:
  ...
  - 32×32px: Full-color Q (blue ring + terracotta tail visible)
  - SVG: Scalable favicon for modern browsers (preferred)
...
- **Background**:
  - Option A: White background, full-color logo (maximum visibility in feed)
  - Option B: Blue-600 background, white logo (stronger brand presence, but less versatile)
  - Recommended: Option A (appears clean in any context)
- **Padding**: 15% internal padding (platforms crop to circles on some displays)
...
- **Design**:
  - Background: White, gray-50, or subtle blue-50
  - Logo: Full-color horizontal version, sized to ~20-25% of cover height
  - Optional: Tagline text ("Evidence-Based Document Validation for TIC") in Inter Semibold
...
- **Colors**: Keep text simple (gray-900 on white), logo provides color
- **Width**: Max 600px total (email client compatibility)
- **Format**: HTML email signature (not image) for better accessibility and lower file size
  - Logo as linked image: `<img src="https://qteria.com/assets/qteria-logo-horizontal-color.png" height="80" alt="Qteria Logo">`
  - Text as HTML: `<span style="font-family: Inter, sans-serif; color: #111827;">`

**Mobile Consideration**: Logo may stack on mobile email clients (logo above text) - test in iOS Mail, Gmail mobile
...
- **Front**:
  ...
  - Name: Center, Inter Semibold 14pt
  - Title: Below name, Inter Regular 10pt, gray-600
  - Background: White
- **Back**:
  - Logo: Small bottom right, 0.3" height
  - Contact info: Left-aligned, Inter Regular 9pt
    - Email
    - Phone
    - Website (qteria.com)
  - Optional: Subtle dot grid pattern (20% opacity) for texture
  - Background: White or gray-50
- **Paper**: Premium matte or uncoated (300gsm+) - avoid glossy (doesn't match brand personality)
...
- Safe zone: Keep text/logo 0.25" from edge
...
- Bullet points: Blue-600 bullets, gray-900 text
- Background: White
...
- Section headers: Inter Semibold 18pt, blue-600
...
4. Body copy (delivers information - gray-900, readable)
...
- **Background**: White navigation bar with gray-200 bottom border
- **Interaction**: Clickable - returns to dashboard/home
...
- **Animation**:
  ...
  - Option B: Static logo with pulsing opacity (50% → 100% → 50%)
  - Recommended: Option B (spinning can feel gimmicky)
- **Text**: Below logo, "Loading validation results..." (Inter Regular 16pt, gray-600)
- **Progress indicator**: Below text if determinate progress (bar or percentage)
- **Background**: White or gray-50
...
- **CTA**: Primary button ("Create Workflow") below body
...
- **Progress indicators**: Blue-600 fill (loading bars, progress circles)
- **Focus states**: 2px blue-600 outline, 2px offset (keyboard navigation)
- **Terracotta accent**: Rarely in UI - perhaps in success confirmation ("Assessment complete! ✓" badge with terracotta check)
...
- **CTA**: "Get Started" (primary button, blue-600)
...
- **Illustration/Icon**: Large centered icon (80px) representing step concept
  - Step 1 (Create Workflow): Stylized document with checklist icon
  - Step 2 (Upload Documents): Upload cloud icon
  - Step 3 (Review Results): Checkmark badge icon
- **Headline**: Step title (Inter Semibold 24pt, gray-900)
- **Body**: Step description (Inter Regular 16pt, gray-600, max-width 480px)
...
- **White space**: Generous - don't overwhelm new users with density
...
- **Body**: Bullet points or paragraphs, Inter Regular 20pt, gray-900
- **Data visualization**: Blue-600 primary, terracotta for emphasis, gray-200 for structure
...
- **Ask slide**: Terracotta-50 subtle highlight for investment amount
...
### Packaging (If Applicable - Future)
...
- Merchandise should feel professional (not playful) - quality materials match brand values
...
**Concept selected instead**: Lettermark Q + wordmark combination gives us both (distinctive Q builds visual equity, wordmark teaches name).
...
**Concept selected instead**: Clean geometric Q letterform (scales perfectly, timeless, professional).
...
**Concept selected instead**: Geometric Q, but with meaningful connection (quality badge + checkmark) not pure abstraction.
...
**Concept selected instead**: Q with subtle validation metaphor (checkmark tail) - hints at function without being literal document icon.
...
**Concept selected instead**: Wordmark with color split (terracotta Q) in Concept C, or Q lettermark + wordmark in Concept A - adds just enough visual interest while maintaining clarity.
...
**Concept selected instead**: Blue as primary + terracotta as strategic accent (logo tail, rare UI highlights) - professional with personality.
...
**Concept selected instead**: Professional blue + muted terracotta (warm but not bright) + grays.
...
**Concept selected instead**: Color palette with restraint (not bright, but not monochrome) - blue + terracotta + grays balance professionalism with warmth.
...
**Concept selected instead**: Limited palette (2-3 brand colors max) for clarity and professional restraint.
...
**Concept selected instead**: Inter (modern sans-serif, highly legible, B2B standard).
...
**Concept selected instead**: Clean Inter (versatile across all sizes, professional, highly readable).
...
**Concept selected instead**: Inter (professional standard, free, excellent quality).
...
**Concept selected instead**: Minimal photography (reserve for marketing website hero sections, about page) - product UI stays clean and photography-free.
...
**Concept selected instead**: Minimal illustrations (maybe for empty states, onboarding in future) - primary visual language is clean typography, iconography, and structured layouts.
...
**Concept selected instead**: Flat colors (blue, terracotta, grays) - timeless, accessible, easy to reproduce. Exception: Concept E explores gradient for specific meaning, but flat versions available.
...
**Concept selected instead**: Minimalist visual style - white space, clean typography, subtle patterns at most. Let content (validation results, evidence links) be the focus, not decorative design.
...
2. **Create logo files** (if design resources available):
   ...
   - [ ] Create all variations:
     ...
     - Horizontal single-color blue
     - Horizontal reversed white
     - Stacked full-color
     - Icon-only Q (full-color, single-color, reversed)
   - [ ] Export formats: SVG (primary), PNG (2x for web, 300dpi for print), PDF (print), ICO (favicon)
...
3. **If working with professional designer**:
   ...
   - [ ] Request deliverables: Logo files (all formats) + basic brand guidelines PDF
...
4. **Implement immediately** (MVP):
   ...
   - [ ] Social media profiles (LinkedIn, Twitter - 400x400px stacked or icon-only)
   - [ ] Email signature (80px height horizontal logo)
   - [ ] Loading screens (120px stacked logo with loading animation)

5. **Create initial marketing assets**:
   - [ ] Business cards (3.5"x2" with logo specs from document)
   - [ ] Pitch deck template (cover + content slide layouts)
   - [ ] One-pager template (for customer handouts, sales)
   - [ ] Email newsletter header (if using email marketing)
...
7. **Test identity in real contexts**:
   ...
   - [ ] Get feedback from TÜV SÜD pilot users (does logo communicate trust/quality?)

8. **Refine based on feedback**:
   - [ ] If Q letterform needs optical adjustment (balance, weight), refine in design software
   - [ ] If terracotta accent isn't working in practice, consider fallback (all-blue single-color default)
   - [ ] If icon-only Q isn't recognizable at 24px, create simplified version
...
11. **Maintain and evolve**:
    ...
    - [ ] Document any modifications (logo refinements, new color usage, new applications)
    - [ ] Plan for potential refresh in 3-5 years (not full rebrand, but refinements)

12. **Protect intellectual property**:
    - [ ] Consider trademark registration for Qteria name
    - [ ] Consider trademark registration for Q letterform logo design (if unique enough)
    - [ ] Register domain variations (qteria.com, qteria.io, qteria.ai if available)
    - [ ] Document logo creation date and designer credit (proof of ownership)
...
16. **Expand to physical brand**:
    - [ ] If attending conferences/events: Design booth graphics, banners, signage
    - [ ] Branded giveaways: USB drives with logo, notebooks, pens (quality items, not cheap swag)
    - [ ] Office branding (if opening physical office): Wall graphics, signage, environmental design
...
- **How it supports**:
  ...
  - **Transparency** (core brand value) is visually supported by semantic colors + icons (never color-only) - accessibility ensures all users can verify results
  - **Professional identity** builds trust in AI results - clean design = careful analysis
...
**Why this visual identity appeals to them**:

1. **Professional aesthetic builds trust**: Blue + gray palette signals seriousness and competence (not playful startup). TIC professionals work in regulated environment - they expect professional tools. Logo as quality badge reinforces "this tool understands quality management."

2. **Clean simplicity matches workflow needs**: Project Handlers run 15-20 assessments/day - they need efficient, uncluttered UI. Visual identity is minimal (not decorative) - lets them focus on validation results, not branding. "Get out of my way so I can work" aesthetic.

3. **Subtle warmth prevents cold enterprise feel**: Terracotta accent adds personality - signals "built by someone who understands you" (domain expertise) not "generic enterprise software from big corp." The Balkan warmth metaphor resonates (approachable expert, not cold system).

4. **Evidence-based design matches evidence-based product**: High legibility typography (Inter) ensures evidence text is readable. Semantic colors + icons ensure accessibility. The design itself is transparent (no tricks, no hiding information) - matches brand value of transparency.

5. **Timeless design suggests stability**: TIC professionals make long-term software decisions (not trendy consumers). Avoiding trends (gradients, maximalism) and choosing timeless elements (geometric Q, classic blue/gray, Inter font) signals "we'll be here in 5 years, you can rely on us."
...
1. **Quality badge logo**: Q as quality badge/magnifying lens = "we catch everything." Directly addresses quality anxiety. Visual metaphor says "we're thorough, organized, careful" - exactly what anxious user needs to see.

2. **Professional restraint**: Clean, organized visual identity mirrors the organized professional user wants to be. Seeing organized, structured brand = feeling of control and order (counters anxiety/frustration).

3. **Warm accent (not cold)**: Terracotta accent prevents feeling like "just another enterprise tool." Adds human touch - "built by someone who understands your stress." Warmth without undermining authority = supportive expert (matches brand personality).

4. **Clear, direct typography**: Inter's high legibility = ease of reading evidence quickly. No confusion, no squinting, no ambiguity. When user is under pressure (15-20 assessments/day), clear typography reduces cognitive load.
...
4. **Balkan warmth vs. Silicon Valley cold**:
   ...
   - **Result**: Users feel understood and supported, not just "using another tool"

5. **Timeless design vs. trendy tech aesthetics**:
   - Many new AI tools use gradients, glassmorphism, "futuristic" styles (will date quickly)
   - Qteria uses flat colors, geometric shapes, classic typography (timeless)
...
- **Visual Style**: Clean, professional, minimal - white space and structure prioritized
- **Context**: New brand launch, building initial recognition, MVP product, pilot customer (TÜV SÜD)
...
- Team growth: Inconsistent brand applications across many people (need stronger guidelines + potential simplification)
...
**Status**: Final - Brand Identity v1.0
**Created**: November 2025
...
- Review brand consistency quarterly (audit website, app, marketing materials for proper logo usage)
...
- **Organized**: Consistent spacing, structured layouts, clear visual hierarchy
- **Supportive**: Terracotta warm accent, approachable (not cold), evidence always visible
...
- Q as quality badge = thoroughness promise (quality gate documents pass through)
...
- NOT cold Silicon Valley tech (Balkan warmth + domain expertise)
...
4. Comprehensive guidelines (expand this doc to team-accessible format) - as team grows

---

**This brand identity system will help TIC professionals recognize and trust Qteria as the evidence-based validation tool built by quality management experts who understand their work.**
