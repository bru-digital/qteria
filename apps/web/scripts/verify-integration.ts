...
const VERCEL_ORIGIN = process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'https://qteria.vercel.app';
...
async function testBackendHealthCheck(): Promise<void> {
  ...
  if (json.environment !== 'production') {
    console.log(`${colors.yellow}  Warning: Environment is '${json.environment}', expected 'production'${colors.reset}`);
  }
...
async function testCORSConfiguration(): Promise<void> {
  ...
  if (allowOrigin !== VERCEL_ORIGIN && allowOrigin !== '*') {
    throw new Error(`CORS origin mismatch. Expected '${VERCEL_ORIGIN}' or '*', got '${allowOrigin}'`);
  }
...
async function testDatabaseConnectivity(): Promise<void> {
  ...
  if (response.statusCode === 401) {
    console.log(`  Database connectivity verified (endpoint returns 401 auth error, not 500 DB error)`);
  } else if (response.statusCode === 200) {
...
async function testResponseTime(): Promise<void> {
  ...
  if (duration > threshold) {
    console.log(`  ${colors.yellow}Warning: Response time ${duration}ms exceeds ${threshold}ms threshold${colors.reset}`);
  } else {
...
async function main(): Promise<void> {
  ...
  console.log(`${colors.blue}║   Qteria Integration Tests - Railway Backend Deployment   ║${colors.reset}`);
  console.log(`${colors.blue}╚════════════════════════════════════════════════════════════╝${colors.reset}\n`);
  ...
  console.log('');
  console.log(`Total: ${total} | ${colors.green}Passed: ${passed}${colors.reset} | ${failed > 0 ? colors.red : colors.reset}Failed: ${failed}${colors.reset}\n`);
