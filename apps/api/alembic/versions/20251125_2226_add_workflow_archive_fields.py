...
def upgrade() -> None:
    ...
    op.add_column(
        "workflows", sa.Column("archived", sa.Boolean(), nullable=False, server_default="false")
    )

    # Add archived_at column (nullable timestamp)
    op.add_column("workflows", sa.Column("archived_at", sa.TIMESTAMP(timezone=True), nullable=True))
